...
export async function requestFileId() {
  const responseData = await PerformQuery(
    QueryMethods.POST,
    QueryPaths.requestFileId(),
    null,
    QueryContentTypes.applicationJson,
    authStore.getJWT
  )
...
export async function uploadCover(collectionId, data) {
  try {
    const responseData = await PerformQuery(
      QueryMethods.POST,
      QueryPaths.uploadCover(collectionId),
      data,
      QueryContentTypes.multipartFormData,
      authStore.getJWT
    )
...
export async function songConvert(songId, fileId) {
  try {
    const responseData = await PerformQuery(
      QueryMethods.POST,
      QueryPaths.songConvert(songId, fileId),
      null,
      QueryContentTypes.applicationJson,
      authStore.getJWT
    )
